...
class PaginationList extends Component {
   constructor(props, context) {
	...
	changePage(page) {
		...
		if (page === prePage) {
			page = (currPage - 1) < pageStartIndex
				? pageStartIndex
				: currPage - 1;
		} else if (page === nextPage) {
			page = (currPage + 1) > this.lastPage
				? this.lastPage
				: currPage + 1;
	...
	makePage() {
		...
		return pages.map(function(page, index) {
			...
			return (
				<PageButton key={ index } changePage={ this.changePage } active={ isActive } disable={ disabled } hidden={ hidden }>
					{ page }
				</PageButton>
			);
		}, this);
